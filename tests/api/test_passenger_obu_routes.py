...
class TestPassengerObuDailyStatsRoute:
    def test_default_fraud_type_is_passenger(self, client):
        ...
        repo.upsert_daily_stat(
            ...
            scanned_count_delta=99,
            suspicious_count_delta=0,
            last_run_at="2026-06-14 12:00:00",
        )
    ...
    def test_date_range_filter(self, client):
        ...
        for day in ("2026-06-12", "2026-06-13", "2026-06-14", "2026-06-15"):
            repo.upsert_daily_stat(
                date=day,
                fraud_type="PASSENGER_USES_TRUCK_OBU_NON_NEW_A",
                scanned_count_delta=1,
                suspicious_count_delta=0,
                last_run_at=f"{day} 12:00:00",
            )

        resp = client.get("/api/audit/passenger-obu/stats/daily" "?from_date=2026-06-13&to_date=2026-06-14")
        body = resp.json()
        assert body["total"] == 2
    ...
    def test_explicit_fraud_type_query(self, client):
        ...
        resp = client.get("/api/audit/passenger-obu/stats/daily?fraud_type=ENTRY_EXIT_MISMATCH")
        body = resp.json()
        assert body["total"] == 1
...
class TestPassengerObuAnomaliesRoute:
    ...
    def test_llm_call_status_passed_through(self, client):
        """details JSON 里的 llm_call_status 字段透传到响应,前端据此渲染四态"""
        _insert_trip_with_result(
            passid="P_LLM_CONFIRMED",
            details_extra={"llm_call_status": "called_confirmed"},
        )
        _insert_trip_with_result(
            passid="P_LLM_REJECTED",
            details_extra={"llm_call_status": "called_rejected"},
        )
        _insert_trip_with_result(
            passid="P_LLM_NOT_CALLED",
            details_extra={"llm_call_status": "not_called"},
        )
        _insert_trip_with_result(
            passid="P_LLM_SERVICE_ERR",
            details_extra={"llm_call_status": "service_error"},
        )

        resp = client.get("/api/audit/passenger-obu/anomalies")
        body = resp.json()
        assert body["total"] == 4
        by_passid = {a["passid"]: a for a in body["anomalies"]}
        assert by_passid["P_LLM_CONFIRMED"]["llm_call_status"] == "called_confirmed"
        assert by_passid["P_LLM_REJECTED"]["llm_call_status"] == "called_rejected"
        assert by_passid["P_LLM_NOT_CALLED"]["llm_call_status"] == "not_called"
        assert by_passid["P_LLM_SERVICE_ERR"]["llm_call_status"] == "service_error"

    def test_missing_details_field_is_none(self, client):
        """老数据 audit_results.details 为 NULL → 所有解析字段 None,前端走 fallback"""
        from apps.api.database.repositories.audit_repository import AuditRepository
        from apps.api.database.repositories.trip_repository import TripRepository
        ...
        assert item["visual_vehicle_type"] is None
        assert item["llm_verified"] is None
        assert item["llm_call_status"] is None  # 新字段也必须降级到 None

    def test_default_sort_by_exit_time_desc(self, client):
        """默认按 exit_time DESC 排序:最近出口的 trip 在最前面"""
        from apps.api.database.repositories.audit_repository import AuditRepository
        from apps.api.database.repositories.trip_repository import TripRepository

        for passid, exit_time in [
            ("P_SORT_MID", "2026-06-15 10:00:00"),
            ("P_SORT_EARLY", "2026-06-15 09:00:00"),
            ("P_SORT_LATE", "2026-06-15 11:00:00"),
        ]:
            trip_id = TripRepository().save_trip(
                {
                    "passid": passid,
        ...
        resp = client.get("/api/audit/passenger-obu/anomalies")
        body = resp.json()
        passids = [a["passid"] for a in body["anomalies"]]
        # 默认按 exit_time DESC:LATE → MID → EARLY
        assert passids == ["P_SORT_LATE", "P_SORT_MID", "P_SORT_EARLY"]

    def test_invalid_sort_by_falls_back_to_exit_time(self, client):
        """非法 sort_by 参数降级到默认 exit_time,不会 500"""
        _insert_trip_with_result(passid="P_FALLBACK")
        resp = client.get("/api/audit/passenger-obu/anomalies?sort_by=DROP%20TABLE")
        assert resp.status_code == 200
        body = resp.json()
        assert body["total"] == 1
