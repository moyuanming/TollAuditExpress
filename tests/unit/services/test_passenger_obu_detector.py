...
class TestEdgeCases:
    def test_both_sides_fail_metadata(self, mock_ai_client):
        """Both sides fail metadata → no hit, no service calls."""
        trip = _base_trip(
    ...
    def test_entry_hit_exit_service_error(self, mock_ai_client):
        """Entry hits, exit service returns error → single-side hit."""
        trip = _base_trip(
            exit_vehicle_type=1,
            exit_vehicle_id="川A99999",
            exit_image_trans="http://example.com/exit_trans.jpg",
    ...
    def test_entry_hit_exit_exception(self, mock_ai_client):
        """Entry hits, exit raises exception → single-side hit."""
        trip = _base_trip(
            exit_vehicle_type=1,
            exit_vehicle_id="川A99999",
            exit_image_trans="http://example.com/exit_trans.jpg",
    ...
    def test_risk_score_single_side(self, mock_ai_client):
        trip = _base_trip()
        mock_ai_client.truck_obu.return_value = _hit_response()
        result = detect_trip(trip)
        assert result["risk_score"] == 0.85

    def test_risk_score_both_sides(self, mock_ai_client):
        trip = _base_trip(
            exit_vehicle_type=1,
            exit_vehicle_id="川A12345",
            exit_image_trans="http://example.com/exit_trans.jpg",
    ...
    def test_entry_media_type_passthrough(self, mock_ai_client):
        """media_type from trip should be in side_check result."""
        trip = _base_trip(entry_media_type=2)
        mock_ai_client.truck_obu.return_value = _hit_response()
        result = detect_trip(trip)
        assert result is not None
