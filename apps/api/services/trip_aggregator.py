...
from apps.api.core.config import DB_HOST, DB_NAME, DB_PASSWORD, DB_PORT, DB_USER
...
def build_gantry_image_url(record: dict) -> str | None:
    """构建门架图片URL（查询式）
...
def get_gantry_image_match(record: dict, time_window: int = None) -> int | None:
    ...
    cursor.execute(sql, (station_id, vehicle_id, occur_time, time_window, occur_time, time_window, occur_time))
        ...
        items.append(
            {
                ...
                "occur_time": r["CAPTURETIME"].isoformat()
                if isinstance(r.get("CAPTURETIME"), datetime)
                else str(r.get("CAPTURETIME") or ""),
                "vehicle_id": r.get("VEHICLEID"),
                "vehicle_color": r.get("VEHICLECOLOR"),
                "image_url": build_gantry_image_url(
                    {"IMAGE_ID": r.get("ID"), "VEHICLEID": r.get("VEHICLEID"), "VEHICLECOLOR": r.get("VEHICLECOLOR")}
                ),
            }
        )
        ...
        items.append(
            {
                ...
                "occur_time": r["OCCURTIME"].isoformat()
                if isinstance(r.get("OCCURTIME"), datetime)
                else str(r.get("OCCURTIME") or ""),
                "vehicle_id": r.get("VEHICLEID"),
                "vehicle_color": r.get("VEHICLECOLOR"),
...
def aggregate_trip(passid: str) -> dict | None:
    """聚合单个行程"""
    ...
    for gr in gantry_records:
        try:
            ...
            if image_id is None:
                logger.debug(
                    "No gantry image match for station=%s vehicle=%s time=%s",
                    gr.get("STATION_ID"),
                    gr.get("VEHICLEID"),
                    gr.get("OCCURTIME"),
                )
...
class TripAggregator:
    ...
    def _schedule_llm_verification_async(self, passid: str | None) -> None:
    ...
    def aggregate_recent_trips(
        self, days: int = 7, limit: int = 100, on_progress: Callable | None = None, run_detection: bool = True
    ) -> int:
        """聚合最近的行程并可选执行视觉检测"""
        from apps.api.database.repositories.trip_repository import TripRepository
        ...
        for i, passid in enumerate(passids):
            ...
            if on_progress:
                on_progress({"current": i + 1, "total": len(passids), "passid": passid, "count": count})
