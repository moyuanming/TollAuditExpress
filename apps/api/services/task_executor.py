...
def _targeted_aggregate(exit_station: str, entry_station: str, days: int, limit: int, trip_repo, aggregator) -> dict:
    ...
    conditions = ["t.LANETYPE = '出口'", "t.MEDIATYPE = 1", "t.OCCURTIME >= DATE_SUB(NOW(), INTERVAL %s DAY)"]
    params = [days]
    if exit_station:
...
def _execute_detect_only(filter_rules: dict, trip_repo) -> dict:
    ...
    for trip in trips:
        ...
        try:
            r = matcher.compare(
                {"image_license": trip.get("entry_image_license")}, {"image_license": trip.get("exit_image_license")}
            )
...
def _execute_passenger_obu_audit(filter_rules: dict, task: dict, trip_repo) -> dict:
    ...
    logger.info("Passenger OBU audit window: %s ~ %s (workers=%d)", start_dt, end_dt, rules["max_workers"])
    ...
    def _detect_one(trip: dict):
        ...
        try:
            ...
            logger.warning("detect_trip failed for passid=%s: %s", trip.get("passid"), e)
            return trip, None
